...
- App 中英切换：界面、Temple 节点、评分说明、修复建议和生成材料均支持 English / 中文。
- 预留大模型 API：内置 Optional LLM 面板，支持 OpenAI-compatible chat completions。
- Temple Mode + Classic Mode：默认展示 2.5D 空间化提交神殿，同时保留稳定双栏表单工具。

## 空间交互

LaunchLens 已实现 `Temple Mode + Classic Mode` 双模式：

- Temple Mode：空间化提交神殿，用 6 个热点引导项目输入、证据检查、评分、大模型润色、材料整理和最终提交，默认作为演示第一屏。
- Classic Mode：保留当前双栏工具布局，用于快速填表、生成材料和稳定提交。

空间交互规格见 `SPACE_INTERACTION_SPEC.md`。V1 已按规格实现 `Oracle LLM` 一级节点、`Final Door` 提交节点、移动端底部导航和 Temple / Classic 顶部切换，并继续复用原有评分、导出和 LLM 逻辑。
...
- 截图：`assets/screenshot.png`、`assets/screenshot-zh.png`、`assets/screenshot-mobile.png`
- 空间背景：`assets/temple-background.png`
...
- 2.5D generated bitmap background
...
- English / Chinese app switch for interface text, Temple nodes, score notes, fix list, and generated outputs.
- Reserved LLM API enhancement through an Optional LLM panel.
- Temple Mode + Classic Mode: a 2.5D spatial submission workflow plus the reliable two-column tool.

## Space Interaction

LaunchLens includes `Temple Mode + Classic Mode`:

- Temple Mode: a spatial Submission Temple with six hotspots guiding builders through intake, evidence checks, readiness scoring, LLM refinement, archive outputs, and final submission. It is the default demo experience.
- Classic Mode: the current two-column tool layout for fast editing, generation, and reliable submission.

See `SPACE_INTERACTION_SPEC.md` for the formal spec. V1 implements Oracle LLM as a first-level node, Final Door for GitHub / Project Wall handoff, mobile bottom navigation, and a Temple / Classic top toggle while preserving the existing scoring, export, and LLM logic.
